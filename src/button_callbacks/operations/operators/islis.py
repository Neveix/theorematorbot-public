...
async def operation_islis_step_1(user_id: int, bot_manager: "TheorematorManager", **kwargs) -> None:
    await unary_operation_step_1(
        user_id = user_id,
        bot_manager = bot_manager,
        next_operation = operation_islis_step_2
    )
    
async def operation_islis_step_2(message: Message, user_id: int, bot_manager: "TheorematorManager", **kwargs) -> None:
    await unary_operation_step_2(
        user_id = user_id,
        message = message,
        bot_manager = bot_manager,
        operation = Matrix.IsLIS
    )